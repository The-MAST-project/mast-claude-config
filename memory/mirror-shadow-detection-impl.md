...
**Open items:** (1) no-shadow validation pending — Arie must find a clean retracted frame; present/absent thresholds (`min_depth=0.08`, `min_prominence=5`, `min_penumbra_half_width=10`) only confirmed on the true-positive path. (2) `reference=` ratio mode (retracted frame) is the cleaner illumination path if the pipeline can supply one at masking time — TBD whether masking must be single-image. (3) thresholds tuned on a single frame.
