...
**Open items:** (1) Shadow detection now validated BOTH sides: 15 mirror-in frames read SHADOW (depth 0.25–0.44) and 19 retracted full-res frames (`Samples1/`) all read CLEAR (depth 0.004–0.007). Depth gate is the true discriminator (prominence 8–11 on the clear frames would false-positive without it). Known limit unchanged: a faint ~4.4% shadow (`full-frame.fits`) is MISSED (fainter than the frame's ~5% illumination structure); single-image can't fix it, reference/flat ratio mode is the path; left as-is. See [[mirror-shadow-optical-center-algorithms]] §F–G. (2) `reference=` ratio mode (retracted frame) is the cleaner illumination path if the pipeline can supply one at masking time — TBD whether masking must be single-image. (3) thresholds tuned on a single frame.
