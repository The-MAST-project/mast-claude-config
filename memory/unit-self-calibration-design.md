...
- PWI4 reality (web-checked 2026-06-18): the official `pwi4_client.py` parses **no temperature** fields (mount/site/focuser/rotator/m3/autofocus only). Temps **exist inside PWI4** (EFA closed-loop mirror/ambient regulation) but the exact `/status` keyword strings are **unconfirmed** — definitive check on a connected unit: `curl -s http://localhost:8220/status | grep -i temp` (read from `status.raw[...]` if present). Candidate sources, best predictor first: (1) EFA primary-mirror sensor; (2) PWI4 `/status` keyword if it exists; (3) PWI4 temperature CSV log; (4) independent ESP32 backplate probe (PWI-version-proof).
