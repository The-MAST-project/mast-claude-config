...
**Credentials:** the fleet's secrets live in `vault.toml` on the share, read through `common.config.vault` (`SecretStr`, lazily loaded, never in a `model_dump`); provisioning's are under `MAST_provisioning/vault/`. Ask there, or ask a person.
...
**mast-share (Samba on mast-ns-control / 10.23.1.181):** share `mast-share`, `security=user`, `guest ok = no`, `map to guest = bad user`, `valid users = mast`. On Windows units it maps to drive **Z:** (`Filer.shared` = `Z:/MAST/<host>` when Z: mapped, else falls back to local `C:/MAST`). `D:` = RAM disk on units (real ramdisk), but on **mast-ns-spec `D:` is a plain local disk** (persistent), not a ramdisk.

**Interim cross-site mount (2026-07-25):** mastw `Z:` maps `\\10.23.1.181\mast-share` (NS control) across the router — route `10.23.3.254 -> 132.76.56.110 -> 10.23.1.181`, ~70 ms RTT, SMB/445 open. Command: `net use Z: \\10.23.1.181\mast-share /user:<user> <password> /persistent:yes`. Slower than a local share but works, until mast-wis-control is installed.

**SSH access:** `mast-ns-spec` and `mastw` accept key `~/.ssh/mast`; `mast00`/`mast02` are password-only (no key) — use paramiko. All fleet units/spec are **Windows** (cmd default shell, git + `py` present; pywin32 only in the unit venv, not system `py`).
